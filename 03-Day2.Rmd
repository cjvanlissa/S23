...
# Compare the fit:
SummaryTable(results, keepCols = c("Filename", "Parameters", "AIC", "BIC", "RMSEA_Estimate", "CFI", "TLI", "SRMR"))
``` 
...
kable_styling(knitr::kable(SummaryTable(results, keepCols = c("Filename", "Parameters", "AIC", "BIC", "RMSEA_Estimate", "CFI", "TLI", "SRMR")), digits = 2, format = "html"), latex_options = c("striped", "scale_down"))
```
...
# Retain only columns with the standardized estimate, pvalue, and 95% CI
row.names(tab) <- NULL
kable_styling(knitr::kable(tab[, c(1, grep("(est_sig|pval|confint)_std", names(tab)))], digits = 2, format = "html"), latex_options = c("striped", "scale_down"))
```
...
tab <- SummaryTable(results, keepCols = c("Parameters", "AIC", "BIC", "RMSEA_Estimate", "CFI", "TLI", "SRMR"), sortBy = NULL)
# Order by BIC
...
row.names(tab) <- NULL
kable_styling(knitr::kable(tab, digits = 2, format = "html"), latex_options = c("striped", "scale_down"))
```

```{r}
# Plot the BICs and annotate with the syntax to see which model is best
library(ggplot2)
qplot(x = 1:11, y = tab$BIC) + geom_line() + geom_text(label = tab$Model, size = 2)

```
```{python, echo=FALSE, results='asis', eval=TRUE, include = show_answers}
print("""
...
m2c_indirect <- m0
m2c_indirect$usevariables <- c(m2c_indirect$usevariables, "AGE1", "GENDER1", "ALCPROB5")
m2c_indirect$VARIABLE <- "CATEGORICAL = ALCPROB5;"
m2c_indirect$MODEL <- paste0(m0$MODEL, "i on AGE1 GENDER1;
                                        i WITH s;
                                        ALCPROB5 on i s;
...
# Replace direct effects with a constrained effect (constrained to 0)
...
row.names(tab) <- NULL
kable_styling(knitr::kable(tab[, c("label", "est_sig", "pval", "confint")], digits = 2, format = "html"), latex_options = c("striped", "scale_down"))
```
...
get_summaries(res_4)[c("AIC", "BIC", "ChiSqM_Value", "RMSEA_Estimate", "CFI", "TLI")]
# Get all columns of results
